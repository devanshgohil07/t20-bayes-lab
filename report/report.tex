...
\begin{center}
{\normalsize\color{ink}
Amartya Jayavel \quad Animesh Kushwaha \quad Ayush Yadav \quad Devansh Gohil\\[4pt]
Nandini Giri \quad Prabhas Sanjay Sah \quad Sunidhi Shreya \quad Yashwardhan Jaggari}
\\[9pt]
{\small\color{muted}
...
practice almost only 0, 1, 2, 4 and 6. Across the \nNdeliveries\ deliveries in our window the mean
...
(Wilde, 2020). Our offset puts the CPL lowest of the four. Both can be true, because they
...
predicts. These batters have a median of \nbigMedianBalls\ balls across the four competitions,
which puts the shrinkage weight $w$ at about \nbigMedianW: the estimate is already mostly their
own record, so partial pooling has little left to add. We report this rather than claim an
...
at rank \nmoverRankRaw\ on the raw list with a strike rate of \nmoverRawExact, and has
$\Pr(\theta > 140) = \nmoverProb$. Batters with strong
...
\textbf{The three plausible priors agree.} All three return the same \ntopTenCommon\ names at
the top of the list, in a slightly different order. Their shortlists hold \nshortScout,
\nshortAnalyst\ and \nshortCfo\ batters, and any two of them overlap with a Jaccard index
between \njaccardLo\ and \njaccardHi. This insensitivity indicates that the data are
informative: with \nNcells\ cells, and long careers for the batters who matter, the likelihood
dominates the prior.

\textbf{The stress test shows where the conclusion would break.} Setting $\tau \approx 2.9$,
which corresponds to the belief that T20 batters are nearly interchangeable, leaves
\nstressKeeps\ of the top ten names in place but reduces the shortlist from \nshortAnalyst\
batters to \nshortStress. The
...
Every number, table and figure in this report is produced by the code in the project
repository; nothing is typed by hand. The repository is public at
\url{https://github.com/devanshgohil07/t20-bayes-lab} and its README documents how to
reproduce the whole pipeline from the raw archives in five commands.
...
\midrule
\texttt{prep/} & \texttt{build\_data.py}: Cricsheet archives to model-ready cells. Assembles the
  T20I aggregate file, drops wides, applies the Full Member filter and the 25-ball floor, and
  prints the bridge matrix. \\
\texttt{analysis/} & The sampler (\texttt{gibbs.py}), seeding, diagnostics, WAIC and PSIS-LOO,
  the four correctness tests, the holdout study, prior sensitivity, and the scripts that write
  every figure, table and quoted number. \\
\texttt{app/} & The interactive site: the same sampler written independently in JavaScript, with
  no libraries and no build step. \\
\texttt{data/} & The model inputs: cells, holdout, the per-ball run distribution and the
  per-league ball profile. \\
\texttt{figs/}, \texttt{tables/} & Figures F1--F10 and tables T1--T7, as generated. \\
\bottomrule
\end{tabular}
\end{center}

The two files worth reading first are \texttt{analysis/gibbs.py}, which is the six full
conditionals derived in Appendix~A written out in under two hundred lines, and
\texttt{app/js/gibbs.js}, which is the same sampler written a second time from the same
derivations. Test 4 checks that the two agree.

\section*{References}
\addcontentsline{toc}{section}{References}

\begin{itemize}[leftmargin=1.4em,itemsep=4pt]
\item ESPNcricinfo (2024). IPL 2025 auction retention rules: six retentions per team, right to
  match returns, Impact Player to stay.\\
  \url{https://www.espncricinfo.com/story/ipl-2025-auction-retention-rules-six-retentions-per-team-right-to-match-returns-impact-player-to-stay-1452396}
\item Gelman, A., Carlin, J.\ B., Stern, H.\ S., Dunson, D.\ B., Vehtari, A.\ and Rubin, D.\ B.
  (2013). \emph{Bayesian Data Analysis}, 3rd edn. Chapman and Hall/CRC. Chapter 5 for the
  hierarchical Normal model, section 11.4 for split $\hat R$.\\
  \url{http://www.stat.columbia.edu/~gelman/book/}
\item Geyer, C.\ J.\ (1992). Practical Markov chain Monte Carlo. \emph{Statistical Science} 7,
  473--483. The initial-positive-sequence estimator used for effective sample size.\\
...
\item Jensen, S.\ T., McShane, B.\ B.\ and Wyner, A.\ J.\ (2009). Hierarchical Bayesian
  ...
  \url{https://doi.org/10.1214/09-BA424}
\item Johnson, A.\ A., Ott, M.\ Q.\ and Dogucu, M.\ (2022). \emph{Bayes Rules! An Introduction
  to Applied Bayesian Modeling}. Chapman and Hall/CRC. Free online edition.\\
  \url{https://www.bayesrulesbook.com}
\item Rushe, S.\ \emph{Cricsheet}: ball-by-ball data for men's T20 cricket. The source of every
  delivery used here.\\
...
\item Stevenson, O.\ G.\ and Brewer, B.\ J.\ (2017). Bayesian survival analysis of batsmen in
  ...
  \url{https://doi.org/10.1515/jqas-2016-0090}
...
\item Wilde, F.\ (2020). CricViz: an analytical guide to the major global T20 leagues.
  \emph{Wisden}, 25 April.\\
  \url{https://www.wisden.com/cricket-news/cricviz-an-analytical-guide-to-the-major-global-t20-leagues}
\item Wilde, F.\ (2020). CricViz: why the IPL is higher quality than T20Is, and the T20 Blast
  is not. \emph{Wisden}, 5 May. The source of the league-standard ordering discussed in
  Section~\ref{sec:results}.\\
  \url{https://www.wisden.com/series/hbl-pakistan-super-league-2019-20/cricket-news/cricviz-why-the-ipl-is-higher-quality-than-t20is-and-the-t20-blast-isnt}
\item Zhang, J.\ and Stephens, M.\ A.\ (2009). A new and efficient estimation method for the
  generalized Pareto distribution. \emph{Technometrics} 51, 316--325. The tail fit inside
  PSIS-LOO.\\
...
\times\ & (\omega^2)^{-(a_\omega+1)}e^{-b_\omega/\omega^2}
